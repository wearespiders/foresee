...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function addUpgradeMessageLink() {
        ...
        if(empty($username) || empty($api_key) || empty($purchase_code)) {
            echo ' <a href="'.$this->url.'">'.__('Dowload new version', "mk_framework").'</a> from CodeCanyon.';

        } else {
            echo ' <a href="'.wp_nonce_url( network_admin_url('options-general.php?page=wpb_vc_settings&vc_action=upgrade')).'">'.__('update now', "mk_framework").'</a> from CodeCanyon.';
        }
    }
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function adminNoticeNewVersion() {
        ...
        if(!empty($version))
            echo '<div class="updated"><p>'.sprintf(__('There is a new version of Visual Composer available. <a href="http://bit.ly/vcomposer" target="_blank">Download version <b>%s</b></a>.', "mk_framework"), $version).'</p></div>';
    }
    ...
    public function registerJavascript() {
        ...
        wp_localize_script( 'wpb_js_composer_js_view', 'i18nLocale', array(
            'add_remove_picture' => __( 'Add/remove picture', "mk_framework"),
            'finish_adding_text' => __( 'Finish Adding Images', "mk_framework"),
            'add_image' => __( 'Add Image', "mk_framework"),
            'add_images' => __( 'Add Images', "mk_framework"),
            'main_button_title' => __( 'Page Composer', "mk_framework"),
            'main_button_title_revert' => __( 'Classic editor', "mk_framework"),
            'please_enter_templates_name' => __('Please enter templates name', "mk_framework"),
            'confirm_deleting_template' => __('Confirm deleting "{template_name}" template, press Cancel to leave. This action cannot be undone.', "mk_framework"),
            'press_ok_to_delete_section' => __('Press OK to delete section, Cancel to leave', "mk_framework"),
            'drag_drop_me_in_column' => __('Drag and drop me in the column', "mk_framework"),
            'press_ok_to_delete_tab' => __('Press OK to delete "{tab_name}" tab, Cancel to leave', "mk_framework"),
            'slide' => __('Slide', "mk_framework"),
            'tab' => __('Tab', "mk_framework"),
            'section' => __('Section', "mk_framework"),
            'please_enter_new_tab_title' => __('Please enter new tab title', "mk_framework"),
            'press_ok_delete_section' => __('Press OK to delete "{tab_name}" section, Cancel to leave', "mk_framework"),
            'section_default_title' => __('Section', "mk_framework"),
            'please_enter_section_title' => __('Please enter new section title', "mk_framework"),
            'error_please_try_again' => __('Error. Please try again.', "mk_framework"),
            'if_close_data_lost' => __('If you close this window all shortcode settings will be lost. Close this window?', "mk_framework"),
            'header_select_element_type' => __('Select element type', "mk_framework"),
            'header_media_gallery' => __('Media gallery', "mk_framework"),
            'header_element_settings' => __('Element settings', "mk_framework"),
            'add_tab' => __('Add tab', "mk_framework"),
            'are_you_sure_convert_to_new_version' => __('Are you sure you want to convert to new version?', "mk_framework"),
            'loading' => __('Loading...', "mk_framework"),
            // Media editor
            'set_image' => __('Set Image', "mk_framework"),
            'are_you_sure_reset_css_classes' => __('Are you sure taht you want to remove all your data?', "mk_framework"),
            'loop_frame_title' => __('Loop settings'),
            'enter_custom_layout' => __('Enter custom layout for your row:', "mk_framework"),
            'wrong_cells_layout' => __('Wrong row layout format! Example: 1/2 + 1/2 or span6 + span6.', "mk_framework"),
        ) );
    ...
    public function jsComposerEditPage() {
        ...
        foreach ($pt_array as $pt) {
            add_meta_box( 'wpb_visual_composer', __('Page Composer', "mk_framework"), Array($this->composer->getLayout(), 'output'), $pt, 'normal', 'high');
        }
    }
