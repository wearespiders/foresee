...
class WPBakeryShortCode_mk_content_box extends WPBakeryShortCode_VC_Column {


	public function mainHtmlBlockParams( $width, $i ) {
		return 'data-element_type="'.$this->settings["base"].'" class="wpb_'.$this->settings['base'].' wpb_content_element wpb_mk_custom_container wpb_content_holder wpb_sortable "'.$this->customAdminBlockParams();
	}
	/*public function mainHtmlBlockParams($width, $i) {
	...
	public function getColumnControls( $controls, $extended_css = '' ) {
		...
		$control_title = sprintf( __( 'Prepend to this %s', "mk_framework"), strtolower( $this->settings( 'name' ) ) );

		$controls_add = ' <a class="column_add" href="#" title="'.$control_title.'"></a>';
		$controls_edit = ' <a class="column_edit" href="#" title="'.sprintf( __( 'Edit this %s', "mk_framework"), strtolower( $this->settings( 'name' ) ) ).'"><i class="mk-icon-pencil"></i></a>';
		$controls_clone = '<a class="column_clone" href="#" title="'.sprintf( __( 'Clone this %s', "mk_framework"), strtolower( $this->settings( 'name' ) ) ).'"><i class="mk-icon-copy"></i></a>';
		$controls_delete = '<a class="column_delete" href="#" title="'.sprintf( __( 'Delete this %s', "mk_framework"), strtolower( $this->settings( 'name' ) ) ).'"><i class="mk-icon-trash"></i></a>';
