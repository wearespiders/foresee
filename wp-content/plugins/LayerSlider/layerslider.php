...
Plugin (Shared on www.Ariyan.org) URI: http://codecanyon.net/user/kreatura/
...
/********************************************************/
	...
	function layerslider_enqueue_admin_res() {
		...
		if(isset($_GET['page']) && strstr($_GET['page'], 'layerslider') != false) {
			...
			// MiniColor
			wp_enqueue_script('minicolor', $GLOBALS['lsPluginPath'].'js/minicolors/jquery.minicolors.js', array('jquery'), $GLOBALS['lsPluginVersion'] );
			wp_enqueue_style('minicolor', $GLOBALS['lsPluginPath'].'js/minicolors/jquery.minicolors.css', array(), $GLOBALS['lsPluginVersion'] );
